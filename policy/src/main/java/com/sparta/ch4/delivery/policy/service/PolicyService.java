...
public class PolicyService {
    ...
    public void initPolicies() {
        ...
        // 사용자 관련 엔드포인트
        policies.put("/api/users/{userId}", Map.of("GET", "authenticated", "DELETE", "authenticated"));
        ...
        // 배송 매니저 관련 엔드포인트
        policies.put("/api/delivery-managers", Map.of("GET", "ROLE_MASTER,ROLE_HUB_MANAGER", "POST", "ROLE_MASTER"));
        policies.put("/api/delivery-managers/{delivery_manager_id}", Map.of(
        ...
        policies.put("/api/companies/{company_id}", Map.of(
        ...
        // 상품 관련 엔드포인트
        policies.put("/api/products", Map.of("GET", "ROLE_HUB_MANAGER,ROLE_HUB_COMPANY", "POST", "ROLE_HUB_MANAGER,ROLE_HUB_COMPANY"));
        policies.put("/api/products/{product_id}", Map.of("GET", "authenticated", "PUT", "ROLE_HUB_MANAGER,ROLE_HUB_COMPANY", "DELETE", "ROLE_HUB_MANAGER,ROLE_HUB_COMPANY"));

        // 주문 관련 엔드포인트
        policies.put("/api/orders", Map.of("GET", "authenticated", "POST", "authenticated"));
        policies.put("/api/orders/{order_id}", Map.of("GET", "authenticated", "DELETE", "ROLE_MASTER,ROLE_HUB_MANAGER"));

        // 배송 관련 엔드포인트
        policies.put("/api/deliveries", Map.of("GET", "authenticated", "POST", "authenticated"));
        policies.put("/api/deliveries/{deliveryId}", Map.of(
                "GET", "authenticated",
        ...
        policies.put("/api/hubs/{hubId}", Map.of("GET", "authenticated", "PUT", "ROLE_MASTER", "DELETE", "ROLE_MASTER"));

        // 허브 이동 관리 관련 엔드포인트
        policies.put("/api/hub-routes", Map.of("POST", "ROLE_MASTER"));
        policies.put("/api/hub-routes/**", Map.of("GET", "authenticated", "PUT", "ROLE_MASTER", "DELETE", "ROLE_MASTER"));

        // 정책을 Redis에 저장하며 1일 동안 캐시 유지
        policies.forEach((endpoint, methodRoles) -> policyCache.put(endpoint, methodRoles, 1, TimeUnit.DAYS));
    }
