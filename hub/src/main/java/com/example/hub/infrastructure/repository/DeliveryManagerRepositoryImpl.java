...
import com.example.hub.domain.model.QDeliveryManager;
import com.example.hub.domain.model.QHub;
import com.example.hub.domain.repository.DeliveryManagerRepositoryCustom;
...
public class DeliveryManagerRepositoryImpl implements DeliveryManagerRepositoryCustom {
    ...
    @Override
    public Page<DeliveryManagerResponse> searchHub(String searchValue, Pageable pageable) {
        // 정렬 기준 설정
        List<OrderSpecifier<?>> orders = getAllOrderSpecifiers(pageable);

        QueryResults<DeliveryManager> results = queryFactory
        ...
        List<DeliveryManagerResponse> content = results.getResults().stream()
                .map(h -> DeliveryManagerResponse.from(h))
    ...
    private BooleanExpression hubNameContains(String hubName) {
        return hubName != null ? deliveryManager.hub.name.contains(hubName) : null;
    }
            ...
            for (Sort.Order sortOrder : pageable.getSort()) {
                ...
                switch (sortOrder.getProperty()) {
                    case "createdAt":
                        ...
                        break;
                    case "updatedAt":
                        orders.add(new OrderSpecifier<>(direction, deliveryManager.updatedAt));
                        break;
                    default:
                        break;
                }
